...
before(async () => {
  ...
  const factory = await ethers.getContractFactory('SampleAP2PTradableToken');
...
describe('P2PTradable', function () {
  ...
  it('mint for test', async function () {
    ...
    tx = await token1.mint();
    await tx.wait();
    result = await token1.ownerOf(tokenId1);
    expect(result).equal(user1.address);

    // for user2
    tx = await token2.mint();
    await tx.wait();
    result = await token2.ownerOf(tokenId2);
    expect(result).equal(user2.address);

    // for user3
    tx = await token3.mint();
    await tx.wait();
    result = await token3.ownerOf(tokenId3);
    expect(result).equal(user3.address);
  });

  it('Attempt to trade non-tradable token', async function () {
    await expect(token2.executeTrade(tokenId2, tokenId1)).revertedWith('TargetTokenId is not on trade');
  });

  it('Attempt to execute trade non-owner token', async function () {
    await expect(token2.executeTrade(tokenId1, tokenId3)).revertedWith('Only the onwer can trade');
  });

  it('Attempt to put trade non-owner token', async function () {
    await expect(token2.putTrade(tokenId1, true)).revertedWith('Only the onwer can trade');
  });

  it('put trade', async function () {
    // user
    tx = await token1.putTrade(tokenId1, true);
    await tx.wait();
    result = await token1.trades(tokenId1);
    expect(result).equal(true);

    tx = await token2.executeTrade(tokenId2, tokenId1);
    await tx.wait();
    result = await token1.trades(tokenId1);
    ...
    result = await token1.ownerOf(tokenId2);
    expect(result).equal(user1.address);
  });
