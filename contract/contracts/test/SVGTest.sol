...
contract SVGTest {
  ...
  function main() external pure returns (string memory output) {
    ...
    samples[1] = SVG.rect(256, 256, 512, 512).fill('yellow');

    samples[2] = SVG.rect(256, 256, 512, 512).fill('yellow').stroke('blue', 10);

    samples[3] = SVG.rect(256, 256, 512, 512).fill('yellow').stroke('blue', 10).transform('rotate(30 512 512)');
