...
abstract contract ERC721WithOperatorFilter is ERC721A, DefaultOperatorFilterer {
  ...
  function approve(address operator, uint256 tokenId) public payable override virtual onlyAllowedOperatorApproval(operator) {
      super.approve(operator, tokenId);
  }

  function transferFrom(address from, address to, uint256 tokenId) public payable override virtual onlyAllowedOperator(from) {
      super.transferFrom(from, to, tokenId);
  }

  function safeTransferFrom(address from, address to, uint256 tokenId) public payable override virtual onlyAllowedOperator(from) {
