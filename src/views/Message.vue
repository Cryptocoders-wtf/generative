<template>
  <div class="home">
    <h2 class="font-bold text-xl m-4">Fully On-chain Message NFT Token.</h2>
    <div class="mx-8">
      <div>
        <div class="text-left">
          Mesasge:
      ...
      <div class="text-left mt-4">
        ...
        <select v-model="color"
                class="resize rounded-md border-2"
                >
      ...
      <NetworkGate :expectedNetwork="chainId">
        <div class="flex items-center justify-center">
          <button
            @click="mint"
            class="w-full mt-4 inline-block rounded bg-green-600 px-6 py-2.5 leading-tight text-white shadow-md transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-lg focus:bg-green-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-green-800 active:shadow-lg"
          >
        ...
        <div class="mt-4">
          <div v-for="(token, k) in tokens" :key="k">
            ...
            <img :src="token.image" class="w-48 border-2 mt-4" />
          </div>
        </div>
      </NetworkGate>
    </div>
